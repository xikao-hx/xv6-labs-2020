...
// add a mapping to the user kernel's page table.
void
...
uint64
ukvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int alloc)
{
  if(newsz >= oldsz)
    return oldsz;
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, alloc);
  }
